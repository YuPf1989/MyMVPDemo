...
public class StatusBarUtil {
    ...
    private static boolean isMIUI6Later(){
        try {
            ...
            String name = ((String) mtd.invoke(null, "ro.miui.ui.version.name"));
            name = name.replace("#[vV]^", "");
    ...
    private static void immersive(Activity activity, int color, @FloatRange(from = 0.0, to = 1.0)float alpha) {
     ...
     */
    private void setMargin(Context context, View view) {
        if (Build.VERSION.SDK_INT >= MIN_API) {
            ViewGroup.LayoutParams lp = view.getLayoutParams();
